						...
						if basicSetting[2] != '0':
							if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
								...
								if bossData[i][2] == '0':
									...
									await client.get_channel(channel).send(f'```자동  횟수 {basicSetting[17]}회 초과! [{bossData[i][0]}] 삭제!```', tts=False)
									print ('자동 미입력 횟수 초과 <' + bossData[i][0] + ' 삭제완료>')
								#await dbSave()
...
async def on_ready():
	...
	await client.change_presence(status=discord.Status.dnd, activity=discord.Game('V4', type=1), afk=False)
