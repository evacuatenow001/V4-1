...
async def on_ready():
	...
	await client.change_presence(status=discord.Status.dnd, activity=discord.Game(name="일하는 중", type=1), afk=False)
	...
	async def on_message(msg):
		...
		if chflg == 0 :
			if msg.content == "!소환":
				...
				await client.get_channel(channel).send('< 텍스트채널 [' + client.get_channel(channel).name + '] 접속완료 >\n< 음성채널 접속 후 [소환] 명령을 사용 하세요 >', tts=False)
